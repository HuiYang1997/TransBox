...
def plot_box_with_shadow(center, offset, color, text, text_pos, line_style='-'):
    low = center - offset
    sizes = 2 * offset
    lower = low.tolist()
    sizes = sizes.tolist()
    rect = mpatches.Rectangle(lower, sizes[0], sizes[1],
                              fill=True, edgecolor=color, linewidth=1.5, alpha=0.2,)


    # # Add shadow effect
    # shadow = path_effects.withStroke(linewidth=5, foreground='yellow', alpha=0.1)
    ...
    elif text_pos == 'lower_left':
        coords = lower[0] + 0.03, lower[1] + 0.04
    else:  # default is upper left
        coords = lower[0] + 0.03, lower[1] + sizes[1] - 0.12

    plt.text(coords[0], coords[1], text, color=color)

def plot_box_boxsqel(center, offset, color, text, text_pos, line_style='-'):
    low = center - offset
    sizes = 2 * offset
    lower = low.tolist()
    sizes = sizes.tolist()
...
device = 'cpu'
model = LoadedModel.from_name('transbox', f'out/transbox', 2, device, best=False)
class_boxes = model.get_boxes(model.class_embeds)
rel_boxes = model.get_boxes(model.relation_embeds)
...
positions = ['lower_left', 'upper_right', ]
for i, c in enumerate(['hasParent', 'hasChild']):
    center, offset = rel_boxes.centers[relations[c]], rel_boxes.offsets[relations[c]]
    plot_box_with_shadow(center, offset, 'black', c, positions[i], 'dotted')
...
plt.gca().autoscale()
plt.savefig('family_transbox.pdf', bbox_inches='tight')
plt.show()
